...
[GADsum_p, ~, stats] = anovan(GADsum,G,'display','off');
c = multcompare(stats);
p_f_GAD = [c(:,6), c(:,4)];
h_GAD = find(c(:,6) < 0.05);

[Nsum_p, ~, stats] = anovan(Nsum,G,'display','off');
c = multcompare(stats);
p_f_N = [c(:,6), c(:,4)];
h_N = find(c(:,6) < 0.05);
...
[~, im] = intersect(Zeids(:,1), lowmood_eid(:,1)); Z{2} = Zs(im, :);
...
[~, iai] = intersect(Zeids(:,1), acuteimpair_eid(:,1)); Z{6} = Zs(iai, :);
...
% print(gcf,'mentalhealth_acuteimpair_bar','-dpng','-r300');
%% Functions
